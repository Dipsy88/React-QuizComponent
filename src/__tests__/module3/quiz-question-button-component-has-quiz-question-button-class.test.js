...
describe('QuizQuestionButton Component', () => {
    it('has a QuizQuestionButton class that extends Component @quiz-question-button-component-has-quiz-question-button-class',
        () => {
                ...
                if (element.type == 'ClassDeclaration') {
                    if (element.id.name == 'QuizQuestion') {
                        if (element.superClass.name == 'Component') {
                            ...
                            assert(false,
                                "We found a class named QuizQuestionButton, but it doesn't extend the Component class.")
                        }
                    }
            ...
            assert(class_declaration_count > 0,
                "We couldn't find any class declarations.")
            assert(class_declaration_count == 1,
                "We found more than one class declaration, but there's only supposed to be one class named QuizQuestionButton.")
        })
    ...
    it('exports the QuizQuestionButton class as default @quiz-question-button-component-has-quiz-question-button-class',
        () => {
            ...
            assert(match != null && match.length > 0,
                "We couldn't find `export default QuizQuestionButton` at the end of your QuizQuestionButton.js file.")
        })
})
