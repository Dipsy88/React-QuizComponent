...
describe('QuizQuestionButton Component', () => {
    it('displays correct button text @quiz-question-button-component-displays-button-text',
        () => {
            ...
            assert(div.querySelector('li button') != null,
                "We can't find a `button` tag that's a child of a single `li` tag in the QuizQuestionButton component's JSX.")
            let button_contents = div.querySelectorAll('li button')[0]
            assert(button_contents.innerHTML
                == quizData.quiz_questions[0].answer_options[0],
                "You're not displaying the correct data from the `button_text` prop in the QuizQuestionButton component's JSX.")
        })
})
