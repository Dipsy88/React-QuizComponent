            ...
            ast['program']['body'].forEach(element => {
                if (element.type == 'ImportDeclaration') {
                    element.specifiers.forEach(el => {
                        if (el.type == 'ImportDefaultSpecifier' && el.local.name
                            == 'React') {
                            react_class_import_found = true
                        } else if (el.type == 'ImportSpecifier'
                            && el.imported.name == 'Component') {
                            component_class_import_found = true
                        }
                    })
                }
            })
            assert(react_class_import_found,
                "You're not importing the React class.")
            assert(component_class_import_found,
                "You're not importing the Component class.")
        });
