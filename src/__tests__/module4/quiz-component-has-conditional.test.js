...
describe('Quiz Component', () => {
    it('renders QuizEnd or QuizQuestion component based on condition @quiz-component-has-conditional',
        () => {
            ...
            if (yallReadyForThis()) {
                assert(quiz.find('QuizQuestion').length == 1 && quiz.find(
                    'QuizEnd').length == 0,
                    "QuizQuestion should be displaying when isQuizEnd is false.")
                quiz.setState(
                    {quiz_position: quizData.quiz_questions.length + 1})
                assert(quiz.find('QuizQuestion').length == 0 && quiz.find(
                    'QuizEnd').length == 1,
                    "QuizEnd should be displaying when isQuizEnd is true.")
            } else {
