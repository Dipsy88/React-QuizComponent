    ...
    it('has a const named isQuizEnd that uses state to determine value @quiz-component-has-is-quiz-end-const',
        () => {
                                    ...
                                    el.body.body.forEach(el2 => {
                                        if (el2.kind == 'const') {
                                            ...
                                            el2.declarations.forEach(el3 => {
                                                if (el3.id && el3.id.name
                                                    == 'isQuizEnd') {
                                                    is_quiz_end_count = is_quiz_end_count
                                                        + 1
                                                    if (el3.init.type
                                                        == 'BinaryExpression') {
                                                        let statement = file.substring(
                                                            el3.init.start - 20,
                                                            el3.init.end)
                                                        ...
                                                        let match = statement.match(
                                                            re)
                                                        assert(match != null,
                                                            "We can't find where you're writing an expression that checks if `this.state.quiz_position - 1` is equivalent to `quizData.quiz_questions.length`.")
                                                        assert(
                                                            match.length == 1,
                                                            "We can't find where you're writing an expression that checks if `this.state.quiz_position - 1` is equivalent to `quizData.quiz_questions.length`.")
                                                    }
            ...
            assert(found_const == 1,
                "We couldn't find a const variable in your Quiz component's render method.")
            assert(is_quiz_end_count == 1,
                "We couldn't find a const variable named `isQuizEnd` in your Quiz component's render method.")
        })
})
