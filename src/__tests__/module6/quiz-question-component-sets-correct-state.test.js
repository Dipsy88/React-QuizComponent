    ...
    it('handleClick method has conditional that checks argument and called clickHandler @quiz-question-component-sets-correct-state',
        () => {
            ...
            try {
                ...
                assert(false,
                    "There's not a method named `handleClick()` in the QuizQuestion class.")
            }
            ...
            try {
                ...
                assert(false,
                    "There's not a method named `showNextQuestion()` in the Quiz class.")
            }
            ...
            assert(JSON.stringify(quizQuestion.state()) == JSON.stringify(
                expectedStateBefore),
                "The QuizQuestion component's state should start out with a key of `incorrectAnswer` that has a value of `false`.")
            try {
            ...
            assert(JSON.stringify(quizQuestion.state()) != JSON.stringify(
                expectedStateAfter),
                "The QuizQuestion components state key of `incorrectAnswer` should not be set to `true` unless the correct answer is clicked.")
            quizQuestion.instance().handleClick('3')
            assert(JSON.stringify(quizQuestion.state()) == JSON.stringify(
                expectedStateAfter),
                "The QuizQuestion components state key of `incorrectAnswer` should be `true` after the button that contains the correct answer is clicked.")
        })
})
