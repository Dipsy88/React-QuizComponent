...
describe('QuizEnd Component', () => {
    it('has a method named `handleResetClick` and a renders a QuizEnd component with a `resetClickHandler` prop @quiz-end-has-reset-click-handler',
        () => {
            ...
            assert(quiz.instance().handleResetClick,
                "There doesn't appear to be a method named `handleResetClick()` in the Quiz component.")

            assert(quiz.find('QuizEnd').props().resetClickHandler != null,
                "The QuizEnd tag in Quiz's JSX doesn't have a `resetClickHandler` property set to the correct value.")

            assert(quiz.find('QuizEnd').props().resetClickHandler.name
                == 'bound handleResetClick',
                "The QuizEnd tag in Quiz's JSX has a `resetClickHandler` property, but the value isn't set to `this.handleResetClick.bind(this)`.")
