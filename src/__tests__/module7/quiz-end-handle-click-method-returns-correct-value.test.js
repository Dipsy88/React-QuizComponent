    ...
    it('handleResetClick method called resetClickHandler @quiz-end-handle-click-method-returns-correct-value',
        () => {
            ...
            try {
                ...
                assert(false,
                    "There's not a method named `handleResetClick()` in the QuizEnd class.")
            }
            ...
            try {
                quizEnd = shallow(<QuizEnd
                    resetClickHandler={mockedPropHandler}/>)
            } catch (e) {
                assert(false, "We weren't able to mount the QuizEnd component.")
            }

            quizEnd.find('a').simulate('click')
            assert(mockedPropHandler.called,
                "QuizEnd's `handleResetClick()` method isn't making a call to `this.props.resetClickHandler()`.")
        })
})
