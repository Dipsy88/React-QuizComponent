    ...
    it('has a QuizQuestion class that extends Component @quiz-question-component-has-quiz-question-class',
        () => {
                ...
                if (element.type == 'ClassDeclaration') {
                    if (element.id.name == 'QuizQuestion') {
                        if (element.superClass.name == 'Component') {
                            ...
                            assert(false,
                                "We found a class named QuizQuestion, but it doesn't extend the Component class.")
                        }
                    }
            ...
            assert(class_declaration_count == 1,
                "We found more than one class declaration, but there's only supposed to be one class named QuizQuestion.")
        })
    ...
    it('exports the QuizQuestion class as default @quiz-question-component-has-quiz-question-class',
        () => {
            ...
            assert(match != null && match.length > 0,
                "We couldn't find `export default QuizQuestion` at the end of your QuizQuestion.js file.")
        })
})
