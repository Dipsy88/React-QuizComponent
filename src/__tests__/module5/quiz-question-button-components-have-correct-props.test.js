    ...
    it('has QuizQuestionButton components with correct props @quiz-question-button-components-have-correct-props',
        () => {
            ...
            let expectedPropsLast = {
                key: 3,
                button_text: quizData.quiz_questions[0].answer_options[quizData.quiz_questions[0].answer_options.length
                - 1]
            }

            if (quizQuestion.find('QuizQuestionButton').length == 1) {
                assert(false,
                    "The QuizQuestion component isn't displaying multiple QuizQuestionButton components.")
                ...
                quizQuestion.find('QuizQuestionButton').forEach((n, index) => {
                    ...
                    assert(n.key() == index,
                        "It doesn't look like the QuizQuestionButton component's `key` props have the correct values.")
                    assert(JSON.stringify(n.props()) == JSON.stringify(
                        expectedProps),
                        "It doesn't look like the QuizQuestionButton component's `button_text` props have the correct values.")
                })
            } else {
                assert(false,
                    "Your QuizQuestion component isn't rendering any QuizQuestionButton components.")
            }
