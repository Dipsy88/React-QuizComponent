...
describe('QuizQuestion Component', () => {
    it('handleClick method has conditional that checks argument and called clickHandler @quiz-question-handle-click-method-returns-correct-value',
        () => {
            ...
            assert(spy2.called == false,
                "It doesn't appear that the conditional logic checking if the `buttonText` is equal to `this.props.quiz_question.answer` in the QuizQuestion's `handleClick` method.")
            try {
                quizQuestion.instance().handleClick('5')
            } catch (e) {
            }
            assert(spy2.called == true,
                "`this.props.showNextQuestionHandler()` is not being called when the `buttonText` is equal to `this.props.quiz_question.answer` in QuizQuestion's `handleClick` method.")
        })
})
